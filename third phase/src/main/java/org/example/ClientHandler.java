...
public class ClientHandler extends Thread {
        ...
        for (User user : User.getUsers()) {
            if (user.getUsername().equals(username)) {
                if (user.getPassword().equals(password)) {
                    ...
                    marketServer.getBankOutPutStream().writeUTF("get_token " + activeUser.getUsername() + " " + activeUser.getPassword());
                    marketServer.getBankOutPutStream().flush();
                    return;
                } else {
    ...
    public void signUpUser(String[] info) throws IOException {
        ...
        marketServer.getBankOutPutStream().writeUTF("create_account " + info[2] + " " + info[3] + " " + info[1] + " " + info[6] + " " + info[6]);
        marketServer.getBankOutPutStream().flush();
    }
    ...
    public void withdraw(String amount) throws IOException {
        if (activeUser.getCredit() >= Integer.parseInt(amount) + Wallet.getLeastAmount()) {
            marketServer.getBankOutPutStream().writeUTF("create_receipt " + token + " " + "deposit " + amount + " " + -1 + " " + activeUser.getAccountId());
            marketServer.getBankOutPutStream().flush();

            String response = marketServer.getBankInputStream().readUTF();
    ...
    public void charge(String amount) throws IOException {
        marketServer.getBankOutPutStream().writeUTF("create_receipt " + token + " " + "withdraw " + amount + " " + activeUser.getAccountId() + " " + -1);
        marketServer.getBankOutPutStream().flush();
    ...
    public void getToken() throws IOException {
        marketServer.getBankOutPutStream().writeUTF("get_token " + activeUser.getUsername() + " " + activeUser.getPassword());
        marketServer.getBankOutPutStream().flush();
        token = marketServer.getBankInputStream().readUTF();
    }
