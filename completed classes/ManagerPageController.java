...
public class ManagerPageController {
    ...
    private boolean phoneNumberIsValid(String phoneNumber) {
        if (phoneNumber.equalsIgnoreCase("back")) {
            return false;
        } else if (!phoneNumber.matches("[+98|0]\\d{10}")) {
    ...
    public void createDiscount() {
        ...
        String code;
        while (!codeIsValid(code = scanner.nextLine().trim())) {
        ...
        while (!(account = scanner.nextLine().trim()).equalsIgnoreCase("end")) {
            ...
            if (userWithNameExists(account)) {
                applyingAccounts.add(User.getAccountByUserName(account));
        ...
        new OffWithCode(code, LocalDateTime.parse(startDate, DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss")), LocalDateTime.parse(endDate, DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss"))
                , Integer.parseInt(percent), Double.parseDouble(max), Integer.parseInt(usesPerOrder), applyingAccounts);
    ...
    private boolean codeIsValid(String code) {
        if (!code.matches("\\d+")) {
            System.out.print("discount code can only include numbers" +
                    "\nplease enter another code:");
            return false;
        } else if (discountWithCodeExists(code)) {
            System.out.print("discount with this code already exists." +
                    "\n please enter another code:");
            return false;
        } else return !code.equalsIgnoreCase("back");
    }
    ...
    public void editDiscountMaxAmount(OffWithCode discount) {
        ...
        if (!(command = scanner.nextLine().trim()).equalsIgnoreCase("back")) {
            discount.setMaxAmount(Double.parseDouble(command));
            System.out.println("discount maximum amount changed successfully");
        }
    }

    public void editDiscountCode(OffWithCode discount) {
        System.out.print("enter new code for discount:");
        while (!codeIsValid(command = scanner.nextLine().trim())) {
            if (command.equalsIgnoreCase("back")) {
                return;
            }
        }
        discount.setOffCode(command);
        System.out.println("discount code successfully changed");
    ...
    public void editDiscountApplyingUsers(OffWithCode discount) {
        ...
        for (User user : discount.getApplyingAccounts()) {
            System.out.println(user.getUserName());
        }
        while ((command = scanner.nextLine().trim()).equalsIgnoreCase("back")) {
            if (command.matches("add (\\w+)") || (matcher = getGroup("add \\w+)", command)).find()) {
                command = matcher.group(1);
                if (discount.getApplyingAccounts().contains(command)) {
    ...
    public void editCategoryParent(Category category) {
        System.out.println("select new parent category");
    ...
    public void addCategory(String name) {
        if (!Category.categoryWithNameExists(name)) {
            ...
            if (command.equalsIgnoreCase("yes")) {
                ...
                while (!Category.categoryWithNameExists(command)) {
                    if (command.equalsIgnoreCase("back")) {
                        return;
                    }
                    System.out.println("this category doesn't exist! choose another parent category");
                    command = scanner.nextLine().trim();
                }
